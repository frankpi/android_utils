...
/**
 * AES加解密，基于网上的代码，但修改了算法，跳过偶数轮的计算，以避免被猜中算法从而爬取我们的后台数据
 * 添加了三个方法：RootEncrypt、RootDecrypt、genKey，请见代码中注释。
 * 
 * @author zhennan
 * 
 */
...
public class AES {
	...
	// main method : do some basic testing
...
//
//
//		String data = "kWmHe8xIsDpfzK4d";
//		System.out.println("Plain: [" + data + "] [" + data.length() + " bytes]");
//
...
//		System.out.println("Encrypted: [" + hex + "] ["+ hex.length() + " bytes]");
//
//		String unencrypted = aes.Decrypt(encrypted);
//		System.out.println("Decrypted: [" + unencrypted + "] ["+ unencrypted.length() + " bytes]");
//		
//		String encrypted = AES.encryptStrToB64Str("plain");
//		System.out.println(encrypted);
//		System.out.println(AES.decryptB64StrToStr(encrypted));
//		

	}

	// do more testing with binary data and salted key
...
//		try {
//
//			aes.setKey(aes.genKey(Long.toHexString(Double.doubleToLongBits(Math.random()))));
//
//			byte[] raw = new byte[0x1111];
//			for (i = 0; i < raw.length; i++)
//				raw[i] = (byte) (i % 0x100);
//			String plain = AdBase64.encode(raw);
//			//System.out.println("plain base64: [" + plain + "] ["+ plain.length() + " bytes]");
//
//			byte[] encrypted = aes.RootEncrypt(plain);
//			//String hex = Util.toHEX(encrypted).replace(" ", "");
//			//System.out.println("Encrypted text (as hex) : [" + hex + "] ["+ encrypted.length + " bytes]");
//
//			aes2.setKey(aes.genKey("random key"));
//
//			String decrypted = aes2.RootDecrypt(encrypted);
//			//System.out.println("decrypted base64: [" + decrypted + "] ["+ decrypted.length() + " bytes]");
//
//			byte[] result = AdBase64.decode(decrypted);
//			for (i = 0; i < result.length; i++) {
//				if (result[i] != (byte) (i % 0x100)) {
//					System.out.format("!!! result mismatch at %x !!!", i);
//					return;
//				}
//			}
//
...
//		}
	...
	private static final byte[] mSalt = { 0x12, 0x24, 0x36, 0x48, 0x5a, 0x6c, 0x7e, 0x0f,
			0x21, 0x42, 0x63, (byte) 0x84, (byte) 0xa5, (byte) 0xc6, 0x7e, (byte) 0xf0};
	private static final int E = 5600;
	private static final int K = 44 * 2;
	public static double EK = 0;
	
	static {
		EK = (double)((double)(E + K) / (double)Math.pow(10, 4));
	}
	/**
	 * generate AES key based on passphase
	 */
	public byte[] genKey(String passphase) {
		
		// magic salt, no meaning, doesn't matter
		// will be used as return byte[] if there's exception below
		//	so it has to be 16 bytes.
		...
		try {
			...
			// iterate 13 times, no meaning, just make it quick, no need to iterate 10000 times
			for (int i = 0; i < 13; i++) {
				md5.reset();
				md5.update(key);
				key = md5.digest(mSalt);
			}
	 ...
	 */
	// zhennan: change this to final, so debug string will not be included in the .class file
...
//		System.out.print("alog[] = n"); for(i=0;i<32;i++) {for(j=0;j<8;j++) System.out.print(Util.toHEX1(alog[i*8+j])+", "); System.out.println();}
	...
	public byte[] encrypt(byte[] plain) {
		...
		for (int r = 1; r < numRounds; r++) {
			if (r%2 == 1) continue;		/// zhennan: skip even rounds
...
//		System.out.println();
	...
	/** Added for Root protocol.
	 *  for binary input data, it's not safe to pick any value as padding byte.
	 *  so we have to base64 the data, then padding ' ' (space).
	 *  and, it's not guaranteed that encrypted data has no '\0',
	 *  so we can not use String to String, we have to add these two method: encrypt string to byte[] and vice versa
	 */ 
	...
	/** Added for Root protocol.
	 *  since protobuf string can only contains UTF-8 or 7-bit ascii, so we need this quick method to encrypt/decrypt string to string
	 */
